...
public class PortfolioValueTest {
    ...
    public void testEquals() {
        String userId = generateId("user@domain.com");
        Instant now = Instant.now().truncatedTo(SECONDS);
        PortfolioValue portfolioValue1 = new PortfolioValue().setUserId(userId).setTimestamp(now).setCredits(10).setMarketValues(Map.of(TWITTER, 12L, YOUTUBE, 23L)).setTotal(1234);
        PortfolioValue portfolioValue2 = new PortfolioValue().setUserId(userId).setTimestamp(now).setCredits(10).setMarketValues(Map.of(TWITTER, 12L, YOUTUBE, 23L)).setTotal(1234);
